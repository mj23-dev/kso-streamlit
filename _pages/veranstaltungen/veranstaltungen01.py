...
gb.configure_column(field='aktivitaten_id', header_name='ID', filter=ag_grid.filters.text, headerCheckboxSelection = True, maxWidth=125)
gb.configure_column(field='datum_titel', header_name='Datum | Titel', pinned='left', filter=ag_grid.filters.text, maxWidth=400)
gb.configure_column(field='format', header_name='Format', filter=ag_grid.filters.text, maxWidth=100)
...
gb.configure_column(field='datum_bis_year', header_name='Year Bis', filter=ag_grid.filters.text, maxWidth=100)
gb.configure_column(field='bundesland', header_name='Place', filter=ag_grid.filters.text, maxWidth=150)
gb.configure_column(field='akt_org', header_name='Organizer', filter=ag_grid.filters.text, maxWidth=200)
gb.configure_column(field='akt_spn', header_name='Sponsor', filter=ag_grid.filters.text, maxWidth=200)
...
gb.configure_column(field='titel', header_name='Titel', filter=ag_grid.filters.text, maxWidth=200)
gb.configure_column(field='adr_full', header_name='Adress', filter=ag_grid.filters.text, maxWidth=400)
gb.configure_columns(["aktivitaten_id", "datum_titel", "format", "datum_bis_year", "datum_bis", "titel"], filterParams={"buttons": ['apply', 'clear']})
gb.configure_column(
    field="agenda_link",
    headerName="Agenda link",
    maxWidth=250,
    cellRenderer=JsCode("""
...
if len(selected_df) > 0:
    ...
    with st.spinner("⏳ Loading ..."):
        ...
        with tab2:
            ...
            gb2.configure_side_bar(filters_panel=True, columns_panel=True)  # Add a sidebar
            ...
            gb2.configure_column(field='email', header_name='Email', filter=ag_grid.filters.text, minWidth=200, maxWidth=200)
            gb2.configure_column(field='juradr_land', header_name='Land', filter=ag_grid.filters.text, maxWidth=150)
            gb2.configure_column(field='juradr_bundesland', header_name='Bundesland', filter=ag_grid.filters.text, maxWidth=150)
            gb2.configure_column(field='juradr_plz_ort', header_name='PLZ-Ort', filter=ag_grid.filters.text, maxWidth=150)
            gb2.configure_column(field='juradr_strasse', header_name='Strasse', filter=ag_grid.filters.text, maxWidth=150)
            gb2.configure_column(field='rechtsform', header_name='Rechtsform', filter=ag_grid.filters.text, maxWidth=150)
            gb2.configure_column(field='product_name_agg', header_name='Product', filter=ag_grid.filters.text, maxWidth=250)
            gb2.configure_column(field='tatigkeitsbeschreibung', header_name='Tatigkeitsbeschreibung', filter=ag_grid.filters.text, maxWidth=250)
            gb2.configure_column(field='last_akt_id', header_name='Last Akt ID', filter=ag_grid.filters.text, maxWidth=120)
            gb2.configure_column(field='akt_datum_titel', header_name='Last aktivitaten', filter=ag_grid.filters.text, maxWidth=200)
            gb2.configure_column(field='heaf', header_name='Heaf', filter=ag_grid.filters.text, maxWidth=100)
            gb2.configure_column(field='product_name_agg', header_name='Uns Product', filter=ag_grid.filters.text,maxWidth=250)
            gb2.configure_column(field='uns_mitg', header_name='Uns Mitg', filter=ag_grid.filters.number, maxWidth=100)
            ...
            gb2.configure_column(field='uns_id', header_name='ID uns', filter=ag_grid.filters.text, maxWidth=120)
