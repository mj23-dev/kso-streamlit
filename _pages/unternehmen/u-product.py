...
query = load_sql(f"{title}/sel_product.sql")
...
gb.configure_column(field='vollname_der_firma', header_name='Full Name', filter=ag_grid.filters.multi, minWidth=100, maxWidth=500)
    ...
    with st.popover("⬇️ Export XLS", use_container_width=True):
        ...
        with col_left_exp:
            if st.button("🔄 Uns.xls", use_container_width=True):
                file_exp1 = f"product_uns_" + datetime.now().strftime('%Y-%m-%d_%H%M%S') + ".xlsx"
                towrite = io.BytesIO()
        ...
        with col_right_exp:
            file_exp2 = f"product_uns_pers_" + datetime.now().strftime('%Y-%m-%d_%H%M%S') + ".xlsx"
...
try:
    if len(selected_df) > 0:
        ...
        if selected_df.iloc[0]['product_name'] and selected_df.iloc[0]['tatigkeitsbeschreibung']:
            col_prod, col_comm = st.columns([0.5, 0.5])
        elif selected_df.iloc[0]['product_name']:
            col_prod = st.empty()
        ...
        try:
            with col_prod:
                if selected_df.iloc[0]['product_name']:
                    expander = col_prod.expander(f"**Products and Services:** {selected_df.iloc[0]['product_name'].split('|')[0]} ... ↩️", expanded=False)
                    expander.write(f"{selected_df.iloc[0]['product_name']}")
                else:
