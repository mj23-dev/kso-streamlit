...
title = 'unternehmen'
st.subheader("🎯 ÖNACE - Klassifikation der Wirtschaftstätigkeiten (ONACE)")

# === 1. Завантаження даних
query = load_sql(f"{title}/sel_onace.sql")
...
gb.configure_column(field='onace_sh_de1', header_name='ONACE L1', filter=True, width=200, rowGroup=True, hide=True)
gb.configure_column(field='onace_sh_de2', header_name='ONACE L2', filter=True, width=200, rowGroup=True, hide=True)
gb.configure_column(field='onace_sh_de3', header_name='ONACE L3', filter=True, width=200, rowGroup=True, hide=True)
gb.configure_column(field='onace_sh_de4', header_name='ONACE L4', filter=True, width=200, rowGroup=True, hide=True)
gb.configure_column(field='onace_sh_de5', header_name='ONACE L5', filter=True, width=200, rowGroup=True, hide=True)
gb.configure_column(field='onace_code5', header_name='ONACE', filter=True, width=100, hide=True)
gb.configure_column(field='cnt_uns', header_name='Cnt Uns', filter=True, aggFunc="sum", sort='desc', width=120)
gb.configure_column(field='vollname_der_firma', header_name='Voller Name', filter=True, minWidth=200, maxWidth=500, flex=0)
gb.configure_column(field='uns_id', header_name='Id', filter=True, minWidth=120, maxWidth=120, flex=0)
    ...
    with st.popover("⬇️ Export XLS", use_container_width=True):
        ...
        with col_left_exp:
            if st.button("🔄 Uns", use_container_width=True):
                file_exp1 = f"u-onace_uns_" + datetime.now().strftime('%Y-%m-%d_%H%M%S') + ".xlsx"
                towrite = io.BytesIO()
        ...
        with col_right_exp:
            file_exp2 = f"u-onace_uns_pers_" + datetime.now().strftime('%Y-%m-%d_%H%M%S') + ".xlsx"
        ...
        try:
            with col_prod:
                if selected_df.iloc[0]['product_name_agg']:
                    expander = col_prod.expander(f"**Produkte von 'Compas':** {selected_df.iloc[0]['product_name_agg'].split('|')[0]} ... ↩️", expanded=False)
                    expander.write(f"{selected_df.iloc[0]['product_name_agg']}")
                else:
                    expander = col_prod.expander("**Produkte von 'Compas':** ❌", expanded=False)
                    expander.write(f"")
        except:
            pass
