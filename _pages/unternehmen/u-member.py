...
selected_land = col2.selectbox("🔍 Select Land", ['--All--'] + sorted(df['rechnungsadr_land'].dropna().unique()), key="selected_land")
if selected_land != '--All--':
    df = df[df['rechnungsadr_land'] == selected_land]
    cnt_filtered = len(df)

selected_bundesland = col2.selectbox("🔍 Select Bundesland", ['--All--'] + sorted(df['rechnungsadr_bundesland'].dropna().unique()), key="selected_bundesland")
if selected_bundesland != '--All--':
    df = df[df['rechnungsadr_bundesland'] == selected_bundesland]
    cnt_filtered = len(df)
...
gb.configure_pagination(enabled=True, paginationAutoPageSize=False, paginationPageSize=1000) #Add pagination
