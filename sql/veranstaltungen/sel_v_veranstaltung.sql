select wv.datum_titel, case when wv.agenda_link = '-' then null else wv.agenda_link end as agenda_link, wv.format,
		count(distinct wv.pers_id) as cnt_pers, count(distinct wv.uns_id) as cnt_uns,
		wv.datum_bis_year, coalesce(wv.bundesland,'-') as bundesland, wv.akt_org, wv.akt_spn,
